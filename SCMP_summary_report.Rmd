...
Interestingly, we didn't find all the contents that SCMP classify in its website, which means that not all of them produces enough engagement. Probably they reach a very specific kind of user that comparing to the whole network doesn't play a role important enough for us to identify it as an isolated topic.
...
After modeling the topic, user cluster and sentiment, we were able to put all those pieces together and split the data into sub - networks.
