...
As said before, SCMP is a important player in Media field, so it is always producing content about several different subjects. As we can see from their web site, they even classify it as:

```{r, out.width='90%', fig.align='center', echo = FALSE}
knitr::include_graphics('C:/Users/gabriela.mourao/Documents/Twitter_template/topic_SCMP.jpg')
```
...
To achieve this goal, we used a popular topic model algorithm called Latent Dirichlet Allocation (available in `topicmodel` R package), which is an unsupervised method for classification of documents. 
As output of this modeling part we got 6 different topics as described below:   
...
All variales were normaliled before running the K-means algorithm. For this case, the best solution that we got had 9 groups:

```{r, out.width='90%', fig.align='center', echo = FALSE}
knitr::include_graphics('C:/Users/gabriela.mourao/Documents/Twitter_template/cluster.png')
```

Our results show that we have some groups that are specialized in one of the topics obtained with the topic modeling part. Which means that during this period in our data their interaction was more focused in one of the topics when comparing to the other ones. For example, the cluster "Hong Kong News - Neutral sentiment" has more posts in the topic "Hong Kong News" than in the other ones. Regardless the topic variable, the users from the specialized clusters ("Hong Kong News - Neutral sentiment", "China News with negative sentiment", "Sports News with negative sentiment", "International News with negative sentiment", "Entertainment News", "USA and China News with negative sentiment") are similar in terms of account information. They have similar average number of followers, tweets and account age, and comparing to the other clusters, they are smaller. In summary, they are just average users with more interest in one of modeled topics than the others.   
The other three groups ("Small users very active in this context", "Big Accounts", "Heavy tweet users") are more generic in terms of topic interest. During the period they didn't show some specialization in some of the topics, they rather talk/ interact with all of them. The difference between them is the size of the account. The group "Small users very active in this context", have the same characteristic from the specialized groups regarding the account size, but they have a different behavior in this context. They are very active and interested in all kind of news and they have the second higher number of tweets per user (average).   
Having these user groups gives a better chance for targeting users with content they are more likely to engage in. This will lead to a greater likelihood for them to follow through visiting SCMP's website for more news; which is way to SCMP capitalize its social media strategy.
...
Our expectation from an agreement analysis is that on average, the sentiment scores of SCMP's posts would match the sentiment scores of the the replies to SCMP's posts. What we see in the agreement visualization below is that majority of 

```{r, out.width='95%', fig.align='center', echo = FALSE}
knitr::include_graphics('SCMP Agreement.png')
```
...
Each of the sub-networks obtained are now relevant enough and more meaningful, since we can easily see the kind of user and sentiment in each of them. Besides that, we can identify new brokers that wasn't possible to identify in the first visualization. Which means that depending on the context (topic) the users that are influencers in the network are different.


```{r, out.width='95%', fig.align='center', echo = FALSE}
knitr::include_graphics('C:/Users/gabriela.mourao/Documents/Twitter_template/networks.png')
```
<br>
<br>     
So, based on this result some strategies can be designed specific for each of those groups. For example, lets focused in USA and China News sub-network:

```{r, out.width='50%', fig.align='center', echo = FALSE}
knitr::include_graphics('C:/Users/gabriela.mourao/Documents/Twitter_template/topic3_v7.png')
```
...
- We could identify two other important users that have high betweenness centrality in the network. They are in the middle of the shortest path between the isolated points and SCMP profile. It is interesting to notice that the sentiment from the "broker's followers" replicates the broker's sentiment spread previously, which means that their opinion is very important and in a certain way followed by the users connected with him.
...
Those isolated groups can be target with prospect strategies. SCMP can try to reach them through advertisement to engage a direct conversation at first moment (convert them as followers). This will help them to engage more with SCMP content and eventually in the future they can be potential clients for its subscription.   
We could identify the same pattern in other networks: "Sports News", "Mainland China News" and "Business News".    
...
- Those broker users are also influencers in the network since their followers tend to replicate their sentiment.   
